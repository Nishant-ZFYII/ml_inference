...
\subsection{Iteration 4: Blended Distillation (Pending)}

Based on the Iteration 3 analysis, the depth loss was redesigned:

\begin{lstlisting}[language=Python]
# Primary: distill from DA3 teacher
loss_da3 = L1(pred, da3_depth)
# Secondary: anchor to GT sensor reading
loss_gt  = L1(pred, gt_depth)
# Blended
L_depth  = loss_da3 + 0.3 * loss_gt
\end{lstlisting}

This maintains DA3 as the \emph{primary} training target (the paper's
claim is knowledge distillation from DA3), while using GT as a
regulariser to stabilise training. Segmentation continues to train on
SAM2 labels.

The \texttt{gt\_blend} parameter defaults to 0.3 and can be set to 0.0
for corridor data where no GT depth exists.
...
Feb 27, 2026 & Iter 4: blended loss $L_{\text{da3}} + 0.3 \cdot L_{\text{gt}}$ (submitted) \\
\bottomrule
\end{longtable}
