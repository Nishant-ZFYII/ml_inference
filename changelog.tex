...
\emph{Pending — results will be available after HPC job completes.}
The script computes per-image RMSE and aggregates mean $\pm$ std
with 95\% CI.  Expected DA3-Small NYU RMSE is $\sim$0.296\,m based
on published benchmarks, though our val split may differ slightly from
the standard Eigen 654-image test set.

\textbf{5.\ Which table does this fall into?}

Table~VII (\texttt{tab:distillation}) — new teacher row:
\texttt{DA3-Small \& --- \& X.XXX\textbackslash,m \& --- \& ---
  \& Teacher (rel.\ depth)}.
...
On HPC at
\texttt{\$SCRATCH/nyu\_depth\_v2/nyu\_da3\_eval/}.
After the job finishes, download to local via:
\texttt{scp np3129@dtn.torch.hpc.nyu.edu:%
  \$SCRATCH/nyu\_depth\_v2/nyu\_da3\_eval/*.json .}

\textbf{8.\ What dataset was used for generating these results?}

NYU Depth V2 (0jl/NYUv2 from HuggingFace, 1449 labelled images,
20\% val split).  The primary rosbag
...
\texttt{Research Paper v1.tex}.  The DA3-Small NYU RMSE value
(mean $\pm$ 95\% CI) will be inserted as a new teacher row once the
HPC job completes and results are downloaded.
