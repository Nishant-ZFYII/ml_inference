...
\textbf{Metric} & \textbf{Value} \\
\midrule
Overall RMSE      & \textcolor{improved}{\textbf{0.596 m}} \\
MAE               & 0.301 m \\
AbsRel            & 0.221 \\
$\delta < 1.25$   & 56.1\% \\
\midrule
...
\midrule
Floor             & \textcolor{improved}{\textbf{0.089 m}} \\
...
\textbf{DA3-SMALL with proper calibration is viable for corridor
navigation.} The key result is near-range RMSE:

\begin{itemize}[nosep]
  \item \textbf{Near (0.3--1.0\,m): 0.158\,m RMSE} --- the collision
    avoidance zone. With 0.8\,m clearance per side, a $\pm$16\,cm
    error is safe for navigation.
  \item \textbf{Floor: 0.089\,m RMSE} --- near-perfect floor distance.
  \item \textbf{Mid (1.0--2.0\,m): 0.486\,m} --- usable for path planning.
  \item \textbf{Far (2.0--4.0\,m): 1.320\,m} --- poor, but far objects
    are less critical for immediate avoidance.
\end{itemize}

The overall RMSE of 0.596\,m is pulled up by the far-range bin. For
corridor navigation specifically, where obstacles are within 1--2\,m,
the effective accuracy is much better than the headline number suggests.

\subsubsection{Distillation Gap Quantified}

\begin{table}[h]
\centering
\caption{Corridor depth: DA3-SMALL (teacher) vs distilled students}
\label{tab:iter9-gap}
\begin{tabular}{lrrrr}
\toprule
\textbf{Model} & \textbf{Params} & \textbf{Overall RMSE} & \textbf{Near RMSE} & \textbf{Floor RMSE} \\
\midrule
DA3-SMALL (median)     & 34.3M  & \textcolor{improved}{\textbf{0.596 m}} & \textcolor{improved}{\textbf{0.158 m}} & \textcolor{improved}{\textbf{0.089 m}} \\
...
\textbf{Status:} DA3-SMALL evaluation complete. DA3METRIC-LARGE
median-scaled evaluation pending (confirms whether 0.35B model has
better underlying depth quality than 34.3M model).
