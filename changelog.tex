...
V5 (EfficientViT-B1 + deployment augmentations) is the \textbf{best result
...
\begin{itemize}[nosep]
  ...
  \item \textbf{Depth structure:} correlation $r = 0.91$ with NYU GT
  \item \textbf{$\delta < 1.25$:} 80.8\% (first time above 80\%)
  \item \textbf{Calibration:} near-perfect (affine correction $<0.5\%$)
\end{itemize}

However, \textbf{V4 remains the best student for corridor deployment}:

\begin{itemize}[nosep]
  \item \textbf{V4 corridor RMSE:} 1.373\,m ($1.77\times$ NYU ratio)
  \item \textbf{V5 corridor RMSE:} 2.186\,m ($3.82\times$ NYU ratio, $+59\%$ regression)
  \item Deployment augmentations improve within-domain accuracy but
    \emph{worsen} cross-domain transfer to corridors
\end{itemize}
...
\begin{itemize}[nosep]
  ...
  \item \textbf{Total improvement V3$\to$V5: RMSE $-51\%$, mIoU $+24.4$pp}
\end{itemize}

\textbf{Key lesson:} Training recipe improvements (augmentations, loss
functions, LR schedules) have reached diminishing returns for corridor
transfer. The dominant bottleneck is now \textbf{training data distribution}.
Adding corridor/hallway data (LILocBench, on-robot collection) is the only
remaining lever to close the $2.3$--$3.7\times$ gap between student models
and DA3-Small on corridor depth.

\textbf{Next steps:}
\begin{enumerate}[nosep]
  \item Train on NYU + LILocBench hallway data using V4 recipe (not V5
    augmentations) to test corridor transfer improvement
  \item DA3-Small direct deployment on Jetson (TensorRT, GerdsenAI wrapper)
    as the interim depth solution
  \item Corridor costmap ablation study: sensor depth vs DA3 depth vs
    student depth for Nav2 local costmap generation
\end{enumerate}

\textbf{Status:} V5 corridor evaluation complete. V4 confirmed as best
corridor student. LILocBench data pipeline in preparation.
