...
> **Per-version detail:** every version below has a dedicated page with architecture, results, and a verdict. See the [model lineage index](models/) for the full set, or jump to a specific version: [V1](models/v1-mobilenet-da2) · [V2](models/v2-kendall-clamp) · [V3](models/v3-da3-berhu) · [V4](models/v4-efficientvit-swap) · [V5](models/v5-deployment-aug) · [V6](models/v6-sun-diode-pretrain) · [V7](models/v7-lilocbench-finetune) · [V8](models/v8-mixed-failed) · [V9](models/v9-corridor-specialist).
...
## V4: EfficientViT-B1 encoder substitution
...
## V6: multi-domain pretraining with NYU fine-tuning
...
## V7: single-domain fine-tuning from V5
...
NYU RMSE more than doubled — a classical catastrophic-forgetting outcome under single-domain fine-tuning. The encoder traded general indoor representations for corridor-specific accuracy. LILocBench RMSE improved sharply in return. The corridor is a narrow distribution (one building, one lighting regime, one floor type), and the model specializes to it quickly under fine-tuning.

---

## V8: joint-domain training ablation
...
Worse than V5 on corridor, barely better than V5 on NYU. The domain gap between NYU (diverse scenes, varying depth ranges) and LILocBench (one corridor, 1-6 m range) is too large for naive mixing to bridge. Multi-task training with explicit domain balancing might help, but it was not pursued — the deployment objective required a corridor specialist, not a universal model.

---

## V9: corridor-specialized student via sequential adaptation
...
**V9 is the production corridor checkpoint for this work.**
...
| `best_depth_v9.pt` | Corridor specialist (from V6 base, production checkpoint) | 0.382 m (LILocBench) |
